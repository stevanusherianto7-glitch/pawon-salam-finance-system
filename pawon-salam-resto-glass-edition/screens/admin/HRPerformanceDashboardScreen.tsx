...
export const HRPerformanceDashboardScreen: React.FC<Props> = ({ onNavigate }) => {
    ...
    return (
        <div className="bg-gray-50 pb-24 min-h-screen overflow-y-auto">
            ...
            {/* === HEADER COMPACT === */}
            <div className="pt-10 pb-16 px-6 rounded-b-[2.5rem] relative overflow-hidden" style={{ background: colors.gradientMain }}>
                ...
                <div className="relative flex items-center gap-4">
                    <div className="relative shrink-0">
                        <img src={user?.avatarUrl} alt="Avatar" className="w-14 h-14 rounded-full border-2 border-white object-cover shadow-md" />
                        <button onClick={handleAvatarClick} className="absolute -bottom-1 -right-1 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow-sm"><Camera size={10} className="text-orange-600" /></button>
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-orange-100 uppercase tracking-widest">Admin Panel</p>
                        <h2 className="text-lg font-bold text-white truncate">{user?.name}</h2>
                        <span className="text-[10px] text-white font-medium bg-white/20 px-2 py-0.5 rounded-full border border-white/20">{getRoleDisplayName(user?.role || UserRole.ADMIN)}</span>
                    </div>
                    <button onClick={logout} className="p-2 bg-white/10 hover:bg-white/20 rounded-full text-white backdrop-blur-sm border border-white/10"><LogOut size={16} /></button>
                </div>
            </div>
            ...
            <div className="px-4 -mt-10 relative z-10 space-y-4">
                ...
                {loading ? (
                    ...
                    <>
                        {eotm && (
                            <button onClick={() => onNavigate && onNavigate('certificate')} className="w-full bg-white rounded-xl p-3 border border-orange-100 shadow-sm flex items-center justify-between relative overflow-hidden active:scale-98">
                                <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-yellow-400 to-orange-500"></div>
                                <div className="flex items-center gap-3 pl-2">
                                    <img src={eotm.avatarUrl} className="w-10 h-10 rounded-full object-cover border border-gray-100" />
                                    <div>
                                        <p className="text-[9px] font-bold text-orange-500 uppercase tracking-wider">Top Performance</p>
                        ...
                        {/* Combined Stats */}
                        <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex divide-x divide-gray-100">
                        ...
                        <button className="w-full py-3 border border-gray-200 rounded-xl text-gray-500 text-xs font-bold flex items-center justify-center gap-2 active:bg-gray-50">
                            <Download size={14} /> Export Laporan Lengkap (PDF)
                        </button>
                    </>
                )}
            </div>
