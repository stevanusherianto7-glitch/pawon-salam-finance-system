      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h2 className="font-bold text-gray-800 text-lg leading-tight">Laporan Jobdesk Harian</h2>
            <p className="text-xs text-gray-500 font-medium mt-0.5">Monitoring & Feedback</p>
          </div>
        </div>
      </div>
                ...
                {relevantEmployees.map(emp => {
                    ...
                    return (
                        <div 
                            ...
                            className={`bg-white p-4 rounded-xl border shadow-sm transition-all ${sub ? 'active:scale-98 cursor-pointer border-gray-100' : 'opacity-70 border-gray-100'}`}
                        >
                            <div className="flex items-center gap-3">
                                <img src={emp.avatarUrl} className="w-10 h-10 rounded-full bg-gray-200 object-cover"/>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <h4 className="font-bold text-gray-800 text-sm">{emp.name}</h4>
                                        {sub ? (
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${progress === 100 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                                                {progress}%
                                            </span>
                                        ) : (
                                            <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-gray-100 text-gray-500">Belum Lapor</span>
                                        )}
                                    </div>
                                    <p className="text-xs text-gray-500">{emp.area} Staff</p>
                                    ...
                                    {sub?.managerNote && (
                                        <div className="mt-2 flex items-center gap-1 text-[10px] text-blue-600 font-medium">
                                            <MessageSquare size={10} /> Ada catatan manager
                                        </div>
                                    )}
                                </div>
                                {sub && <ChevronRight size={16} className="text-gray-300"/>}
                            </div>
                        </div>
                    );
                })}
                  ...
                  <div className="p-4 overflow-y-auto">
                      <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                              ...
                              <p className="font-bold text-gray-800">{selectedSubmission.completedTaskIds.length} Tugas</p>
                          </div>
                      </div>

                      <div className="space-y-3 mb-6">
                          <label className="text-xs font-bold text-gray-500 uppercase">Catatan / Feedback Manager</label>
                          <textarea 
                              className="w-full p-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-200 transition-all"
                              rows={3}
                              placeholder="Berikan apresiasi atau catatan perbaikan..."
                              value={feedbackNote}
                              onChange={e => setFeedbackNote(e.target.value)}
                          />
                          <p className="text-[10px] text-gray-400 italic">*Catatan ini akan tampil di dashboard karyawan.</p>
                      </div>

                      <button 
                          onClick={handleSaveFeedback}
                          disabled={isSavingFeedback}
                          className="w-full py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all flex justify-center items-center gap-2"
                      >
                          {isSavingFeedback ? 'Menyimpan...' : <><Save size={18}/> Kirim Catatan</>}
                      </button>
                  </div>
              </div>
          </div>
