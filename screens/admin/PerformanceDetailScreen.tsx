...
export const PerformanceDetailScreen: React.FC<Props> = ({ employeeId, onBack }) => {
  ...
  return (
    <div className="bg-gray-50 pb-24">
       {/* Header */}
       <div className="pt-8 pb-12 px-6 rounded-b-[2.5rem] shadow-md relative z-0" style={{ background: colors.gradientMain }}>
          <div className="flex items-center gap-3 mb-4 text-white">
            <button onClick={onBack} className="p-2 bg-white/20 rounded-full hover:bg-white/30">
               <ArrowLeft size={20} />
            </button>
            <div className="flex-1">
                <h2 className="text-xl font-bold">Detail Performa</h2>
                <p className="text-orange-100 text-sm opacity-90">Monitor kinerja & evaluasi</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
             <img src={employee.avatarUrl} className="w-14 h-14 rounded-full border-2 border-white bg-gray-200" alt={employee.name} />
             <div className="text-white">
                <h3 className="font-bold text-lg leading-none mb-1">{employee.name}</h3>
                <div className="flex items-center gap-2 text-xs text-orange-100">
                   <span className="bg-white/20 px-2 py-0.5 rounded flex items-center gap-1"><User size={10}/> {employee.department}</span>
                   <span className="bg-white/20 px-2 py-0.5 rounded font-bold">{employee.area}</span>
                </div>
             </div>
          </div>
       </div>
       ...
       <div className="px-4 -mt-6 relative z-10">
          <div className="bg-white rounded-2xl shadow-lg p-2 mb-4 border border-orange-50">
             {/* Month Selector */}
             <div className="flex justify-between items-center p-2 mb-2 border-b border-gray-100">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-gray-700">{getMonthName(selectedMonth)} {selectedYear}</span>
                </div>
                <button 
             ...
             <div className="flex bg-gray-100 p-1 rounded-xl">
                <button 
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex justify-center items-center gap-2 ${activeTab === 'REVIEWS' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-500'}`}
                >
                   <Star size={14} /> Evaluasi Bulanan
                </button>
                <button 
                  onClick={() => setActiveTab('DAILY')}
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex justify-center items-center gap-2 ${activeTab === 'DAILY' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}
                >
                   <TrendingUp size={14} /> Monitoring Harian
                </button>
             </div>
          </div>
             ...
             {!isLoading && activeTab === 'DAILY' && (
                 filteredSnapshots.length === 0 ? (
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="grid grid-cols-12 gap-2 bg-gray-50 p-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                        ...
                        {filteredSnapshots.map(snap => {
                            ...
                            return (
                                <div key={snap.id} className="grid grid-cols-12 gap-2 p-3 border-b border-gray-50 items-center hover:bg-gray-50 transition-colors">
                                     <div className="col-span-3">
                                         <p className="text-xs font-bold text-gray-800">{dateObj.getDate()}</p>
                                         <p className="text-[10px] text-gray-400">{dateObj.toLocaleDateString('id-ID', {month: 'short'})}</p>
                                     </div>
                                     <div className="col-span-3 text-center">
                                         <span className={`font-bold text-xs px-2 py-0.5 rounded ${getScoreColor(snap.punctualityScore) === '#ef4444' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                                     ...
                                     <div className="col-span-3 text-center">
                                         {hasChecklist ? (
                                             <div className="flex flex-col items-center">
                                                <span className="font-bold text-xs text-blue-600">{checklistAvg}</span>
                                                <CheckSquare size={10} className="text-blue-400 mt-0.5"/>
                                             </div>
                                         ) : (
                                     ...
                                     <div className="col-span-3 text-right">
                                         <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${snap.status === 'AUTO_GENERATED' ? 'bg-gray-50 text-gray-500 border-gray-200' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
