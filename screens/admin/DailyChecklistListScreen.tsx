...
export const DailyChecklistListScreen: React.FC<Props> = ({ onBack, onNavigate }) => {
    ...
    return (
        <div className="bg-gray-50 pb-24">
            {/* Header */}
            <div className="pt-12 pb-16 px-6 rounded-b-[3rem] shadow-md relative z-0 overflow-hidden" style={{ background: colors.gradientMain }}>
                {/* Watermark */}
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4 text-white">
                        ...
                        <h2 className="text-xl font-bold">Daily Checklist Performance</h2>
                    </div>
                ...
                <div className="bg-white p-1 rounded-xl shadow-sm flex border border-orange-50">
                    {(['ALL', EmployeeArea.FOH, EmployeeArea.BOH] as const).map(area => (
                        <button
                            ...
                            className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all ${selectedArea === area ? 'bg-orange-50 text-orange-600 shadow-sm' : 'text-gray-400 hover:bg-gray-50'}`}
                        >
                            {area === 'ALL' ? 'Semua' : area}
                        </button>
                    ))}
                ...
                <div className="space-y-3">
                    {filteredEmployees.length === 0 ? (
                        <div className="text-center py-10 flex flex-col items-center justify-center text-gray-400">
                            ...
                            <p>Tidak ada karyawan yang sesuai filter.</p>
                        </div>
                    ) : (
                        filteredEmployees.map(emp => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between active:scale-98 transition-transform ${isLoadingStatus ? 'opacity-70 pointer-events-none' : ''}`}
                                >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className="relative">
                                            <img src={emp.avatarUrl} className="w-12 h-12 rounded-full bg-gray-200 object-cover border border-gray-100" alt={emp.name} />
                                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white flex items-center justify-center ${emp.area === 'FOH' ? 'bg-blue-500' : 'bg-red-500'}`}>
                                                <span className="text-[8px] text-white font-bold">{emp.area[0]}</span>
                                            </div>
                                        </div>
                                        <div className="min-w-0">
                                            <h4 className="font-bold text-gray-800 text-sm truncate">{emp.name}</h4>
                                            <p className="text-xs text-gray-500 font-medium truncate">{emp.department}</p>
                                            <div className="mt-1">
                                    ...
                                    <div className="flex items-center gap-3 pl-2">
                                        {score ? (
                                            <div className="text-right">
                                                <span className="text-lg font-bold block leading-none" style={{ color: getScoreColor(score) }}>
                                                    {score}
                                                </span>
