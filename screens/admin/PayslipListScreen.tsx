...
export const PayslipListScreen: React.FC<Props> = ({ onBack, onNavigate }) => {
  ...
  return (
    <div className="bg-gray-50 pb-24">
       {/* Header */}
       <div className="pt-12 pb-16 px-6 rounded-b-[3rem] shadow-xl relative z-0 overflow-hidden" style={{ background: colors.gradientMain }}>
           ...
           <div className="relative z-10">
               <div className="flex items-center gap-3 text-white mb-3">
                   <button onClick={onBack} className="p-2 bg-white/20 rounded-full hover:bg-white/30 transition-colors border border-white/10 backdrop-blur-sm">
                       <ArrowLeft size={20} />
                   </button>
                   <div className="flex items-center gap-2">
                       <div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm border border-white/10"><Banknote size={20} /></div>
                       <h2 className="text-xl font-bold font-heading">Payroll Management</h2>
                   </div>
               </div>
               <p className="text-orange-100 text-sm px-1 font-medium">Kelola dan kirim slip gaji karyawan</p>
           </div>
       </div>
       ...
       <div className="px-4 space-y-4 -mt-10 relative z-10">
           ...
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-3">
               <div className="flex gap-2">
                   <select value={month} onChange={e => setMonth(Number(e.target.value))} className="bg-gray-50 border rounded-lg px-3 py-2 text-sm outline-none flex-1">
                       {[...Array(12)].map((_, i) => <option key={i} value={i+1}>{new Date(0, i).toLocaleDateString('id-ID', {month:'long'})}</option>)}
                   </select>
                   <select value={year} onChange={e => setYear(Number(e.target.value))} className="bg-gray-50 border rounded-lg px-3 py-2 text-sm outline-none w-24">
               ...
               <div className="relative">
                   ...
                   <input 
                     ...
                     className="w-full pl-9 p-2.5 bg-gray-50 border rounded-xl text-sm outline-none focus:border-orange-500"
                     value={searchTerm}
           ...
           {user?.role === UserRole.HR_MANAGER && (
               <button 
                 ...
                 className="w-full py-3 bg-green-600 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform"
               >
           ...
           <div className="space-y-3">
               {isLoading && payslips.length === 0 ? (
                   ...
                   <div className="text-center py-10 text-gray-400 bg-white rounded-xl border border-dashed border-gray-200">
                       ...
                       <p>Belum ada data slip gaji untuk periode ini.</p>
                   </div>
               ) : (
                   filteredPayslips.map(slip => {
                       ...
                       return (
                           <div 
                             ...
                             className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center active:scale-98 transition-transform"
                           >
                               <div className="flex items-center gap-3">
                                   <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200 overflow-hidden">
                                       {emp?.avatarUrl ? <img src={emp.avatarUrl} className="w-full h-full object-cover"/> : <User size={20} className="text-gray-400"/>}
                                   </div>
                                   <div>
                                       <p className="font-bold text-gray-800 text-sm">{emp?.name || 'Unknown'}</p>
                                       <p className="text-xs text-gray-500">{emp?.department}</p>
                                   </div>
                               </div>
                               <div className="text-right">
                                   <p className="font-bold text-green-600 text-sm">Rp {slip.netSalary.toLocaleString('id-ID')}</p>
