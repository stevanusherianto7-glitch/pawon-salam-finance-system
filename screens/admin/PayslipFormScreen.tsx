...
export const PayslipFormScreen: React.FC<Props> = ({ onBack, payslipId, isNew, initialMonth, initialYear }) => {
      ...
      <div className="mb-3">
          ...
          <div className="relative">
              ...
              <input 
                ...
                className={`w-full pl-9 p-2.5 border rounded-xl text-sm outline-none focus:border-orange-500 transition-colors ${color === 'green' ? 'bg-green-50 text-green-700' : color === 'red' ? 'bg-red-50 text-red-700' : 'bg-white'}`}
              />
          </div>
      </div>
  );
  ...
  return (
    <div className="bg-gray-50 pb-24">
       <div className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm px-4 py-4 flex items-center gap-3">
          <button onClick={onBack} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
              <ArrowLeft size={20} className="text-gray-600"/>
          </button>
          <div className="flex-1">
              <h2 className="font-bold text-gray-800 text-lg leading-tight">{isNew ? 'Buat Slip Gaji' : 'Edit Slip Gaji'}</h2>
              {selectedEmployee && <p className="text-xs text-gray-500">{selectedEmployee.name} • {selectedEmployee.department}</p>}
          </div>
       </div>
       ...
       <div className="p-4 max-w-lg mx-auto space-y-6">
           ...
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-4">
               {isNew ? (
                   <div>
                       <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Pilih Karyawan</label>
                       <select 
                         className="w-full p-3 border rounded-xl text-sm bg-gray-50 outline-none"
                         onChange={e => {
                   ...
                   <div className="flex items-center gap-3 bg-orange-50 p-3 rounded-xl border border-orange-100">
                       <img src={selectedEmployee?.avatarUrl} className="w-10 h-10 rounded-full bg-white"/>
                       <div>
               ...
               <div className="grid grid-cols-2 gap-3">
                   <div>
                       ...
                       <select 
                         ...
                         className="w-full p-2.5 border rounded-xl text-sm bg-gray-50 outline-none"
                       >
           ...
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
               <h3 className="font-bold text-green-700 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider"><CheckCircle2 size={16}/> Pendapatan</h3>
               ...
               <div className="bg-green-50 p-3 rounded-xl flex justify-between items-center border border-green-100">
                   <span className="text-xs font-bold text-green-800 uppercase">Total Pendapatan</span>
                   <span className="font-bold text-green-800">Rp {formData.totalEarnings?.toLocaleString('id-ID')}</span>
               </div>
           </div>

           {/* 3. Deductions */}
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
               <h3 className="font-bold text-red-700 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider"><AlertCircle size={16}/> Potongan</h3>
               <div className="grid grid-cols-2 gap-3">
               ...
               <div className="bg-red-50 p-3 rounded-xl flex justify-between items-center border border-red-100">
                   <span className="text-xs font-bold text-red-800 uppercase">Total Potongan</span>
                   <span className="font-bold text-red-800">- Rp {formData.totalDeductions?.toLocaleString('id-ID')}</span>
               </div>
           </div>

           {/* 4. Net & Notes */}
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
               <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 text-center mb-4">
                   <p className="text-xs font-bold text-orange-500 uppercase mb-1">Gaji Bersih (Take Home Pay)</p>
                   <p className="text-3xl font-bold text-orange-700">Rp {formData.netSalary?.toLocaleString('id-ID')}</p>
                   <p className="text-[10px] text-gray-500 mt-1">Dibayarkan tgl: {new Date(formData.payDate!).toLocaleDateString('id-ID')}</p>
               </div>

               <div className="space-y-3">
                   <div>
                       <label className="text-xs font-bold text-gray-500 mb-1 block">Pesan untuk Karyawan</label>
                       <textarea 
                         disabled={!canEdit}
                         rows={2}
                         className="w-full p-3 border rounded-xl text-sm bg-gray-50 outline-none"
                   ...
                   <div>
                       <label className="text-xs font-bold text-gray-500 mb-1 block">Catatan Internal HR</label>
                       <textarea 
                         disabled={!canEdit}
                         rows={1}
                         className="w-full p-3 border rounded-xl text-sm bg-gray-50 outline-none"
           ...
           {canEdit && (
               <div className="flex gap-3 pt-2">
                   <button 
                     ...
                     className="flex-1 py-3.5 border-2 border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50 active:scale-95 transition-all"
                   >
                       Simpan Draft
                   </button>
                   <button 
                     ...
                     disabled={isSubmitting}
                     className="flex-1 py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all"
                   >
                       Siap Kirim
                   </button>
               </div>
           ...
           {user?.role === UserRole.HR_MANAGER && formData.status === 'READY_TO_SEND' && (
               ...
               >
                   <Send size={18}/> Kirim Slip Gaji Sekarang
               </button>
           )}
