...
export const EmployeePayslipDetailScreen: React.FC<Props> = ({ payslipId, onBack }) => {
  ...
  const Row = ({ label, value, isTotal = false, isDeduction = false }: { label: string, value: number, isTotal?: boolean, isDeduction?: boolean }) => (
      <div className={`flex justify-between py-2 border-b border-dashed border-gray-100 ${isTotal ? 'font-bold text-gray-900 pt-3 border-gray-200 border-solid' : 'text-sm text-gray-600'}`}>
          <span>{label}</span>
  ...
  return (
    <div className="bg-gray-100 min-h-screen pb-10">
       <div className="sticky top-0 z-10 bg-white shadow-sm px-4 py-4 flex items-center gap-3">
           ...
           <h2 className="font-bold text-lg">Detail Slip Gaji</h2>
       </div>
           ...
           <div className="bg-white rounded-2xl shadow-lg overflow-hidden relative">
               <div className="p-6 space-y-6">
                   <div className="flex justify-between items-end border-b pb-4">
                       <div>
                           <p className="text-xs text-gray-500 uppercase font-bold">Periode</p>
                       ...
                       <button 
                         ...
                         className="flex items-center gap-2 px-3 py-2 bg-orange-500 text-white rounded-lg text-xs font-bold active:scale-95 transition-all shadow-md hover:bg-orange-600 disabled:opacity-60"
                       >
                   ...
                   <div>
                       <h3 className="text-xs font-bold text-green-600 uppercase mb-2 tracking-wider">Pendapatan</h3>
                   ...
                   <div>
                       <h3 className="text-xs font-bold text-red-600 uppercase mb-2 tracking-wider">Potongan</h3>
                   ...
                   <div className="bg-orange-50 p-4 border border-orange-100 rounded-lg text-center">
                       <p className="text-xs font-bold text-orange-500 uppercase">Gaji Bersih Diterima</p>
                       <p className="text-3xl font-bold text-gray-800 mt-1">Rp {currentPayslip.netSalary.toLocaleString('id-ID')}</p>
                   </div>
                   
                   {currentPayslip.notesForEmployee && (
                       <div className="text-center px-4">
                           <p className="text-xs text-gray-500 italic">"{currentPayslip.notesForEmployee}"</p>
                       </div>
                   )}
               </div>
           </div>
